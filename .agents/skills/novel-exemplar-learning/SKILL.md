...
description: Exemplar Learning / 样本投喂学习；用于作者投喂觉得写得好的章节、开头、章末、爽点、对白、题材或节奏样本，拆解承诺-延迟-兑现、情绪机制和可迁移技法；不复制内容、不入 canon。
---
...
- 爆款机制：熟悉母题如何被包装出新意、承诺-延迟-兑现间隔、主角收益曲线、情绪波形、大战后余波。
- 反工业化：哪些技巧被藏进人物动作和场景，哪些地方没有掉书袋。
...
- `digest_id`
- `source_input_id`
- `created_from`：external_sample / author_reference / model_output。
...
- `promise_delay_payoff`
- `emotion_curve_mechanics`
- `payoff_human_need`
- `freshness_mechanism`
- `non_transferable_elements`：不能学的角色、设定、桥段、专有名词、原句。
- `source_boundary`：说明样本中哪些内容必须排除在当前项目之外。
- `risk_if_copied`
...
- `next_commands`：如需迁移，写 `agent: use novel-learning-transfer with <digest_id>`。
...
- 需要提炼题材卖点 -> `novel-premise-alchemy`。
- 需要迁移爽点机制 -> `novel-payoff-architecture`。
- 需要迁移节奏波形 -> `novel-emotion-curve`。
...
- 是否只提炼技法，而不是借用内容？
- 是否写清哪些东西不能迁移？
- `digest_id` 和 `source_input_id` 是否能让后续迁移准确引用？
- 是否能指导下一章的具体写法？
- 是否拆出承诺、延迟、兑现和情绪波形？
- 是否有作者确认入口？
- 是否保留学习价值但避免抄袭风险？
