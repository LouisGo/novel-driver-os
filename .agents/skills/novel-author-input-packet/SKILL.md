...
description: 中文网文 作者输入包 技能。用于把作者灵感、正文、正稿、人设、设定、世界观、大纲、留白、文风反馈、废案、AI 稿反馈或章节审稿请求等原始输入整理成统一输入包，保留原文证据、权威等级、任务意图和确认需求；不用于分析正文质量或写正史。
---
...
- `recommended_actions`
- `task_intent`：如 ingest_only、chapter_quality_review、opening_review、style_review；没有则可省略。
- `created_at`
...
5. 如果作者是在请求审稿，不要把审稿结论写进 packet；只记录任务意图并推荐 `novel-human-chapter-intake` 或 `novel-style-miner`。
