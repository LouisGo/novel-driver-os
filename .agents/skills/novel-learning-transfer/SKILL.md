...
description: Learning Transfer / 学习迁移；用于把 learning_digest 转成当前项目的题材炼金、爽点架构、情绪曲线、章节 brief、写作约束或 variant briefs；只迁移机制，不复制内容、不定稿。
---
...
4. 把技法转成 6 类产物：
   - `premise_moves`：题材碰撞、新意包装、现实情绪映射。
   - `payoff_moves`：承诺、铺垫、延迟、兑现、代价、后果。
   - `emotion_curve_moves`：压力、蓄势、兑现、缓冲、转向。
...
- 是否能用于下一章生成或改稿？
- 是否能转给 `novel-premise-alchemy`、`novel-payoff-architecture`、`novel-emotion-curve` 或 `novel-chapter-brief-builder`？
- 是否便于后续比稿评估？
